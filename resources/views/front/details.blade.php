				...
				<a href="{{ route('front.details', $news->slug) }}" class="card-from-author">
					<div
						...
						<div class="flex flex-col gap-[6px]">
							<p class="line-clamp-2 font-bold">{{ substr($news->name, 0,50) }}{{ strlen($news->name) > 50 ? '...' : '' }}</p>
							<p class="text-xs leading-[18px] text-[#A3A6AE]">{{ $news->created_at->format('M d, Y') }} • {{ $news->category->name }}t</p>
						</div>
					</div>
				</a>
                @empty
